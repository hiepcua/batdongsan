...
class CLS_CONTENTS{
    ...
    public function listTable($strwhere="",$page){
        ...
        while($rows=$objdata->Fetch_Assoc()){
            ...
            $category = $this->getCatName($cat_id);
	...
	function Update_Product($tag,$id){
		$objMysql=new CLS_MYSQL;
		$item=','.$id.',';
		$sql="UPDATE tbl_tags SET pids=Replace(pids,'$item', ',')  WHERE pids LIKE '%,$id,%'";
		$objMysql->Exec($sql);
		...
		$sql="UPDATE tbl_tags SET pids=CONCAT(ifnull(pids,''),$id,',') WHERE code in ('$tag')";
		$objMysql->Exec($sql);
	}
    public function addNew(){
        ...
        $sql="INSERT INTO tbl_contents (`category_id`,`code`,`thumb`,`list_tagid`,`list_conid`,`cdate`,`author`,`ishot`,`isactive`,`title`,`intro`,`fulltext`,`meta_title`,`meta_key`,`meta_desc`) VALUES ";
        $sql.="('".$this->CategoryID."','".$this->Code."','".$this->Thumb."','";
        $sql.=$this->ListTags."','".$this->ListConId."','".$this->Cdate."','".$this->Author."','".$this->isHot."','".$this->isActive."','".$this->Title."','".$this->Intro."','";
        ...
        if($result){
			$objdata->Exec('COMMIT');
			$this->Update_Product($this->Tags,$ids);
			return $result;
		}
		else	$objdata->Exec('ROLLBACK');
    }
    ...
    public function Update(){
		...
		if($result){
			$objdata->Exec('COMMIT');
			$this->Update_Product($this->Tags,$this->ID);
			return $result;
		}
		else
			$objdata->Exec('ROLLBACK');
    }
